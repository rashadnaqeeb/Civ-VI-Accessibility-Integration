...
// streamed system voice (speech.mm) has voice, rate and volume. prism's
// registry is built with the Macaw screen reader's plugin on top of prism's
// own backends, so Macaw, VoiceOver and AVSpeech are the prism backends.
...
// only when its best backend is a screen reader (Macaw or VoiceOver), so
// screen reader users opt into auto or prism; a handler that fails to start
...
// itself, as a screen reader stops speaking on a key press. A screen reader
// backend (Macaw, VoiceOver) is left alone, since it does this on its own. With wait
