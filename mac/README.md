...
- `prism`: prism (https://github.com/ethindp/prism), a library over screen reader and speech APIs, with the `PrismBackend` setting naming a backend or `auto` for its best available. On a Mac, prism offers a VoiceOver backend ("VoiceOver" in its registry) and an AVSpeech backend, and the router adds the Macaw screen reader's backend ("Macaw") described below.
- `auto`: prism only when its best backend is a screen reader (Macaw or VoiceOver), otherwise the system voice.

The default is the system voice rather than auto because on a Mac without a screen reader running prism's best backend is AVSpeech, with the gaps described above. Screen reader users opt into auto or prism. A handler that fails to start falls back to the system voice. Prism 0.18.2 is linked statically from its release archive with `-force_load`, because its backends register through static initializers, and needs IOKit.

Macaw (https://github.com/aaronr7734/macaw), a macOS screen reader with an NVDA-style speech API documented in its `docs/speech-api.md`, has no backend inside prism. It ships a prism plugin in its own bundle and links it at a fixed path at every launch, `~/Library/Application Support/Macaw/prism/libMacawPrismPlugin.dylib`, a universal binary built against prism 0.18.2 and plugin ABI 1. `speechRouter.cpp` builds prism's registry through `prism_registry_builder_new` plus `prism_registry_builder_add_library` on that path and freezes it into the `PrismConfig`; the builder starts with prism's built-in backends, so the registry is Macaw, VoiceOver and AVSpeech when the plugin is present and unchanged when the path does not exist (prism leaves the builder untouched on a failed add, verified with a host probe). The plugin's backend is named "Macaw", ranks at priority 110 above VoiceOver, and reports itself supported at runtime only while the reader's socket exists, so `prism_registry_create_best` picks Macaw exactly when Macaw runs. The plugin is a client of Macaw's speech socket: an interrupting line is sent at the `now` priority, a queued line at `normal`, `is_speaking` counts the utterances the reader has not yet reported finished or interrupted, and the plugin reconnects on its own after a Macaw restart. Macaw is treated as a screen reader in two places: the auto handler accepts it, and the key monitor does not stop it, since Macaw interrupts its own speech on every key. If a line fails with `PRISM_ERROR_BACKEND_NOT_AVAILABLE` (Macaw quit), the router re-selects the handler from the settings, which lands on VoiceOver, AVSpeech or the system voice, and speaks the line there; a Macaw started after that, or installed after the game launched (the registry is frozen at first use), is picked up by changing a Speech setting or restarting the game.
